 ...
 */
class DeactivationSampleTest extends TestCase
{
    private $sender;
    private $receiver;
    private $service;
    ...
    public function testSendDeactivationMessage()
    {
        $randomId = rand(1000, 9999);
        $message = DeactivationBuilder::create()
            ->setHeader($this->sender, $this->receiver, $this->sender)
            ->setTimestamp(date("Ymdhis", time()))
            ->setDossierId("$this->sender-$randomId")
            ->setOriginalNetworkOperator($this->receiver)
            ->setCurrentNetworkOperator($this->sender)
